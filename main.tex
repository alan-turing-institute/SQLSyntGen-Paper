...
\author{
Carlos Gavidia-Calderon$^1$, Steve Harris$^2$, Markus Hauru$^1$, Carsten Maple$^1$^,$^3$, Iain Stenson$^1$, and \\ May Yong$^1$ \\~\\
...
The number of research papers on synthetic data has surged in the last five years due to its use in addressing data scarcity and sensitivity in machine learning tasks. From 2023 to mid-2024 alone, 25,600 papers have been published on this topic. These studies span various domains, including computer vision, natural language processing, and healthcare \cite{Fonseca23}, often focusing on the generation, evaluation, and application of synthetic data, particularly using GANs \cite{10.3390/math10152733}. Originally research-driven, these methods are now being applied in real-world scenarios, with publications examining the associated challenges \cite{jordon2022synthetic}.

In this section, we discuss our lessons learnt from developing an SDG to meet a specific real-life application: that of enabling the sharing of hospital information. We surmise that there are two challenges: a) the lack of generators developed for relational data and b) the lack of explainability in privacy preserving mechanisms.  

\cite{Fonseca23} states that the development of synthetic generators often overlook tabular data, a common data format used in industrial applications. Similarly, we find that that generators also overlook the relational data format, possibly because of the foreign key constraints satisfaction criteria. This is a problem because hospital datasets are often stored in relational formats. 

Explainability in synthetic data generators is a crucial issue for custodians of sensitive data, especially in hospitals. The lack of explainability undermines discussions between hospital data stakeholders, including both staff and patients. 

One discussion impacted by the lack of explainability is that of maintaining a balance between privacy guarantees and the utility of the synthetic data. While ensuring that synthetic data generators do not leak sensitive information is essential, explaining the privacy preservation mechanisms involved can be complex. Furthermore, the processes used by generators based on GANs and deep neural networks are opaque, making it difficult to assure stakeholders of the synthetic data's reliability and safety. Finally, both generators and metrics (e.g., fidelity, diversity) used to evaluate the quality of synthetic data  are not easily interpretable.

This paper introduces \textsc{SqlSynthGen} (SSG) to facilitate the sharing of hospital data. While SSG effectively meets this goal, it has several limitations, some of which can be improved:

\paragraph{Lack of Model Discovery:} Unlike GANs-based \cite{10.3390/math10152733} or Bayesian-based generators \cite{Cai2023}, SSG cannot discover underlying models or relationships within the data. Users must predetermine the models they want to incorporate into the outputs. Consequently, algorithms trained on SSG outputs are unlikely to transfer successfully to real data unless users have already specified the underlying data model.

\paragraph{Need to Ensure Security:} The design of SSG, which includes copying vocabulary tables in their entirety and executing SQL statements on real data based on user configurations, makes it a powerful tool. However, these features introduce risks of user errors. Accidentally copying tables with sensitive data could lead to catastrophic data leaks. Executing SQL statements without proper access controls could also damage real patient information.

\paragraph{Lack of Evaluation:} SSG allows users to selectively disclose information used to shape synthetic data outputs. Since each piece of information is individually chosen, there is an opportunity here to iteratively fine-tune the balance between fidelity and privacy using an evaluation tool like TAPAS \cite{houssiau2022tapas}.
