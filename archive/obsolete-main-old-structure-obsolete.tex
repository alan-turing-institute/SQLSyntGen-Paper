...
Current mechanisms for addressing data sharing problems \todo{Could mechanisms for addressing data sharing problems be replaced with just mechanisms for data sharing?} are necessarily \todo{I would soften the claim to ``can be'', and probably add a citation} a) time-consuming, b) linked to inadequate privacy protection measures, c) cause unnecessary friction to analysis, d) cannot be scaled to large datasets and/or e) cannot be applied to data in relational databases, a format in which many large datasets are stored.
...
\begin{itemize}
    ...
    \item \textbf{Valid datasets} imply that the values in the synthetic dataset are correct and correct eg. date of births are valid dates. \todo{Markus: I’m not sure what “correct” means here, and I think there has been some sort of typo here.}
...
High fidelity data is likely to be more useful (analytic valuable), but also has an increased risk of identification (disclosure risk). The three factors: a) fidelity, b) utility and c) privacy are all factors to be decided according to different use cases. f\todo{Markus: I don’t think we’ve discussed the distinction between fidelity and utility up to this point, and it may not be obvious. Could explain it or cite something that explains it?}
...
\todo{Markus: This paragraph I think is important and could be improved. I bet a lot of people, reading the stuff above it, are thinking “what use are these datasets that aren’t at all medically plausible?” and this paragraph answers that. I think the needs of software testing and data exploration could maybe be detailed a bit more.}

\todo{Markus: Maybe there could be more of an emphasis somewhere earlier on when we talk about different data sharing methods on the fact that synthetic data is not meant to, and can not, fully replace giving access to the real data. That eventually often full data access is required, but that synthetic data helps a lot on the way there, with things like testing and exploration and bringing code to data rather than data to code.}
...
\begin{enumerate}[leftmargin=*]
    ...
    \todo{Markus: DP needs a citation.}
    ...
    \todo{Markus: Does saying "privacy is not always technically feasible" risk having some people think that it might be bypassed/ignored? Depends on the target audience.}
